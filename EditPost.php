...
<div class="container-xxl bg-white p-0">
    ...
    <?php
    $id = $_GET['id'];
    $query = mysqli_query($sql, "SELECT * FROM posts WHERE id='$id'");
    ...
    <div class="container-xxl py-5">
        <h1 class="mb-3">Editare postare</h1>
        <form action="edit.php" method="post" enctype="multipart/form-data">
            <div class="mb-3">
                <label for="id" class="form-label">ID produs</label>
                <input name="id" class="form-control" id="id" placeholder="Adauga denumirea pensiunii" value="<?php echo $id; ?>" readonly>
            </div>

        <div class="mb-3">
                <label for="denumirea" class="form-label">Denumirea pensiunii</label>
                <input name="denumirea" class="form-control" id="denumirea" placeholder="Adauga denumirea pensiunii" value="<?php echo $denumirea; ?>">
        </div>

        <div class="mb-3">
            <label for="imagine" class="form-label">Imaginea pensiunii</label>
            <input name="imagine" class="form-control" id="imagine" placeholder="Link-ul de la imaginea pensiunii" value="<?php echo $imagine; ?>">
        </div>

        <div class="mb-3">
            <label for="tipul" class="form-label">Tip pensiune</label>
            <input name="tipul" class="form-control" id="tipul" placeholder="Tipul pensiunii" value="<?php echo $tipul; ?>">
        </div>

        <div class="mb-3">
            <label for="culinara" class="form-label">In baza culinara</label>
            <input name="culinara" class="form-control" id="culinara" placeholder="Adauga tipul de culinarie" value="<?php echo $culinara; ?>">
        </div>

        <div class="mb-3">
            <label for="hobby" class="form-label">In baza hobby-urilor</label>
            <input name="hobby" class="form-control" id="hobby" placeholder="Adauga tipul de hobby" value="<?php echo $hobby; ?>">
        </div>

        <div class="mb-3">
            <label for="localitate" class="form-label">Localitatea pensiunii</label>
            <input name="localitate" class="form-control" id="localitate" placeholder="Adauga localitatea pensiunii" value="<?php echo $localitate; ?>">
        </div>

        <div class="mb-3">
            <label for="pretul" class="form-label">Pretul</label>
            <input name="pretul" class="form-control" id="pretul" placeholder="Adauga pretul pensiunii" value="<?php echo $pretul; ?>">
        </div>

        <button type="submit" value="INCARCA DATELE" name="submit" class="btn btn-primary mb-3">Update</button>
        <!-- Footer Start -->
